...
Ad Hoc On-Demand Distance Vector Energy Aware(AODV_EO)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand 
Distance Vector Energy Aware (AODV_EO) protocol. The implementation is based on 
:rfc:`3561`.
...
Uppsala University. The implementation of energy aware version of Ad Hoc
On-Demand Distance Vector protocol by Erhan Turan and Özay Keküllüoğlu of
Hacettepe University.

Model Description
*****************

The source code for the AODV_EO model lives in the directory `src/aodv_eo`.

Design
++++++

Class ``ns3::aodv_eo::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::aodv_eo::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::aodv_eo::RouteInput``,
...
this functionality are attributes of ``ns3::aodv_eo::RoutingProtocol``. 
...
AODV_EO discovers routes on demand.  Therefore, the AODV_EO model buffers all 
packets while a route request packet (RREQ) is disseminated. 
A packet queue is implemented in aodv_eo-rqueue.cc. A smart pointer to 
