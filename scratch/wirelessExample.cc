...
int main (int argc, char *argv[])
{
 ...
 // int nBottomNodes = rows * columns;
  ...
  int X = (columns-1)*hSpacing+1;              // X and Y are the dimensions of the field X=4*201=804
  //int Y = 300; 
  int packetsize = 500;
  double factor = 1.0;  // allows slowing down rate and extending runtime; same total # of packets
  int endtime = (int)100*factor; //100
  double speed = (X-1.0)/endtime;  //8.03
 ...
 // Ptr<ListPositionAllocator> positionAlloc2 = CreateObject<ListPositionAllocator> (); // Top nodes
  ...
  for(int j=0; j< columns; j++){
   ...
   mobilityFixedNodes.SetMobilityModel ("ns3::WaypointMobilityModel");
  ...
  mobile.SetMobilityModel("ns3::WaypointMobilityModel");        // no Attributes
  mobile.Install(mover);
  cvmm = mover->GetObject<WaypointMobilityModel> ();
