...
AodveeExample::InstallApplications ()
{
  ...
  RngSeedManager::SetRun(150);
  Ptr<UniformRandomVariable> y=CreateObject<UniformRandomVariable>();


  //V4PingHelper ping (interfaces.GetAddress (size - 1));
  //  V4PingHelper ping (interfaces.GetAddress (91));
  V4PingHelper ping (interfaces.GetAddress ((double)y->GetInteger(0,size)));
  ping.SetAttribute ("Verbose", BooleanValue (true));

//  ApplicationContainer p = ping.Install (nodes.Get (0));
  ApplicationContainer p = ping.Install (nodes.Get ((double)y->GetInteger(0,size)));
  p.Start (Seconds (0));
  //p.Start (Seconds ((double)y->GetInteger(0,(int)totalTime)));
